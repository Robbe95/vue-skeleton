...
function handleIconClicked(): void {
  if (props.hasHidden)
...
<template>
  <div class="relative">
    ...
    <button
      v-if="hidden"
      class="absolute top-0 z-50 flex h-screen flex-col items-center justify-center overflow-hidden bg-primary text-white transition-all duration-500"
      :class="{
        'w-8': !isOutside,
        'w-0': isOutside,
    ...
    <div
      class="sticky top-0 flex h-screen w-full flex-col justify-between overflow-auto bg-primary text-white transition-all duration-500"
      :class="{
        'w-80': !minified,
        'w-32': minified,
      ...
      <div>
        <div class="flex items-center justify-between gap-4 p-4">
          ...
          <button @click="handleIconClicked">
            <ChevronRightIcon
              v-if="hasMinified" class="w-3" :class="{
                'rotate-180 transition-transform': minified,
              }"
            />
            <AppIcon v-if="hasHidden" icon="close" class="w-3" />
          </button>
        </div>
        <AppDivider />
